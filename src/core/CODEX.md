...
|--------|---------|-----------|
| `agent/` | Codex SDK compatibility layer wrapper | `CodexService` (incl. fork session tracking), `SessionManager`, `QueryOptionsBuilder` (incl. `resumeSessionAt`), `MessageChannel`, `customSpawn` |
...
| `storage/` | Persistence layer | `StorageService`, `SessionStorage`, `CCSettingsStorage`, `CodexdianSettingsStorage`, `McpStorage`, `SkillStorage`, `SlashCommandStorage`, `VaultFileAdapter` |
...
const service = new CodexService(plugin, vaultPath);
...
// Sessions: SDK-native (~/.codex/projects/) + metadata overlay (.meta.json)
...
- `CodexService` must be disposed on tab close (abort + cleanup)
- `SessionManager` handles SDK session resume via `sessionId`
- Fork uses `pendingForkSession` + `pendingResumeAt` on `CodexService` to pass `resumeSessionAt` to SDK; these are one-shot flags consumed on the next query
