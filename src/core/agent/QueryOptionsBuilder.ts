 ...
 *
 * Extracts options-building logic from CodexService for:
...
import type { CodexdianSettings, PermissionMode } from '../types';
 ...
 * Passed to builder methods to avoid direct dependencies on CodexService.
 */
...
export class QueryOptionsBuilder {
  ...
  static needsRestart(
    ...
    // Note: Permission mode is handled dynamically via setPermissionMode() in CodexService.
  ...
  static buildPersistentQueryConfig(
    ...
    return {
      ...
      allowedExportPaths: ctx.settings.allowedExportPaths,
      settingSources: ctx.settings.loadUserCodexSettings ? 'user,project' : 'project',
  ...
  static buildPersistentQueryOptions(ctx: PersistentQueryContext): Options {
    ...
    const options: Options = {
      ...
      pathToCodexExecutable: ctx.cliPath,
      settingSources: ctx.settings.loadUserCodexSettings
        ? ['user', 'project']
        : ['project'],
      env: {
        ...process.env,
    ...
    options.spawnCodexProcess = createCustomSpawnFunction(ctx.enhancedPath);
  ...
  static buildColdStartQueryOptions(ctx: ColdStartQueryContext): Options {
    ...
    const options: Options = {
      ...
      abortController: ctx.abortController,
      pathToCodexExecutable: ctx.cliPath,
      // User settings may contain permission rules that bypass Codexdian's permission system
      settingSources: ctx.settings.loadUserCodexSettings
        ? ['user', 'project']
        : ['project'],
      env: {
  ...
  private static applyExtraArgs(options: Options, settings: CodexdianSettings): void {
