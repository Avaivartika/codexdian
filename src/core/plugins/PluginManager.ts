/**
 * PluginManager - Discover and manage Codex CLI plugins.
 *
...
import type { CodexdianPlugin, InstalledPluginEntry, InstalledPluginsFile, PluginScope } from '../types';

const INSTALLED_PLUGINS_PATH = path.join(os.homedir(), '.codex', 'plugins', 'installed_plugins.json');
const GLOBAL_SETTINGS_PATH = path.join(os.homedir(), '.codex', 'settings.json');
...
export class PluginManager {
  ...
  private plugins: CodexdianPlugin[] = [];
    ...
    if (installedPlugins?.plugins) {
      for (const [pluginId, entries] of Object.entries(installedPlugins.plugins)) {
        ...
        if (!Array.isArray(entries)) {
          new Notice(`Codexdian: plugin "${pluginId}" has malformed entry in installed_plugins.json (expected array, got ${typeof entries})`);
        }
  ...
  private async loadProjectSettings(): Promise<SettingsFile | null> {
    const projectSettingsPath = path.join(this.vaultPath, '.codex', 'settings.json');
    return readJsonFile(projectSettingsPath);
  }

  getPlugins(): CodexdianPlugin[] {
    return [...this.plugins];
  }
