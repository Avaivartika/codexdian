 ...
 *
 * Manages the .codex/settings.json file in Codex CLI compatible format.
 * This file is shared with Codex CLI CLI for interoperability.
 *
 ...
 *
 * Codexdian-specific settings go in codexdian-settings.json.
 */
...
import { CODEXDIAN_ONLY_FIELDS } from './migrationConstants';
import type { VaultFileAdapter } from './VaultFileAdapter';

/** Path to CC settings file relative to vault root. */
export const CC_SETTINGS_PATH = '.codex/settings.json';

/** Schema URL for CC settings. */
const CC_SETTINGS_SCHEMA = 'https://json.schemastore.org/codex-settings.json';

function hasCodexdianOnlyFields(data: Record<string, unknown>): boolean {
  return Object.keys(data).some(key => CODEXDIAN_ONLY_FIELDS.has(key));
}

/**
 * Check if a settings object uses the legacy Codexdian permissions format.
...
export class CCSettingsStorage {
   ...
   */
  async save(settings: CCSettings, stripCodexdianFields: boolean = false): Promise<void> {
    ...
    if (await this.adapter.exists(CC_SETTINGS_PATH)) {
      try {
        ...
        // Only strip Codexdian-only fields during explicit migration
