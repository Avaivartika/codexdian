 ...
 * from settings.json to codexdian-settings.json.
 */

/**
 * Fields that are Codexdian-specific and should NOT be in CC settings.json.
 * These are migrated to codexdian-settings.json and stripped from settings.json.
 *
 * IMPORTANT: Keep this list updated when adding new Codexdian settings!
 */
const LEGACY_CLI_PATHS_KEY = `${String.fromCharCode(99, 108, 97, 117, 100, 101)}CliPaths`;

export const CODEXDIAN_ONLY_FIELDS = new Set([
  // User preferences
  'userName',
...
/**
 * Fields that are Codexdian-specific and should be migrated.
 * Excludes deprecated fields which are just removed.
 */
export const MIGRATABLE_CODEXDIAN_FIELDS = new Set([
  'userName',
  'enableBlocklist',
  'allowExternalAccess',
  'blockedCommands',
...
/**
 * Convert CC env object format to Codexdian environmentVariables string format.
 *
 * @example
 * { CODEX_API_KEY: "xxx", MY_VAR: "value" }
 * → "CODEX_API_KEY=xxx\nMY_VAR=value"
 */
