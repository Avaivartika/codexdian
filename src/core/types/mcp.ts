/**
 * Codexdian - MCP (Model Context Protocol) type definitions
 *
...
/** Extended server configuration with Codexdian-specific options. */
export interface CodexdianMcpServer {
...
/** Extended config file with Codexdian metadata. */
export interface CodexdianMcpConfigFile extends McpConfigFile {
  _codexdian?: {
    /** Per-server Codexdian-specific settings. */
    servers: Record<
      string,
      {
        enabled?: boolean;
...
export const DEFAULT_MCP_SERVER: Omit<CodexdianMcpServer, 'name' | 'config'> = {
  enabled: true,
  contextSaving: true,
};
