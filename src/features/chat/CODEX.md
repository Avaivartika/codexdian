...
Main sidebar chat interface. `CodexdianView` is a thin shell; logic lives in controllers and services.

## Architecture

```
CodexdianView (lifecycle + assembly)
...
```
User Input → InputController → CodexService.query()
                                      ↓
...
```typescript
// CodexService created on first query, not on tab create
...
- `CodexdianView.onClose()` must abort all tabs and dispose services
