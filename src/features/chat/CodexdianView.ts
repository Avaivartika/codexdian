...
import { getContextWindowSize, VIEW_TYPE_CODEXDIAN } from '../../core/types';
import type CodexdianPlugin from '../../main';
import { CODEX_ICON_ID, LOGO_SVG } from './constants';
import { TabBar, TabManager, updatePlanModeUI } from './tabs';
import type { TabData, TabId } from './tabs/types';

export class CodexdianView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: CodexdianPlugin) {
    ...
    // overwritten by prototype patching. Hover Editor patches CodexdianView.prototype.load
  ...
  private applyThemeMode(): void {
    ...
    const mode = this.plugin.settings.themeMode ?? 'auto';
    if (mode === 'light') {
  ...
  private wireEventHandlers(): void {
    ...
    // navigating away when Codexdian is open as a main-area tab.
