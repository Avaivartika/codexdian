...
import type { ApprovalCallbackOptions, CodexService } from '../../../core/agent';
...
export class InputController {
  ...
  private getAgentService(): CodexService | null {
  ...
  async sendMessage(options?: {
    ...
    const contentEl = msgEl.querySelector('.codexdian-message-content') as HTMLElement;
    ...
    streamController.showThinkingIndicator(
      ...
      isCompact ? 'codexdian-thinking--compact' : undefined,
    );
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText('\n\n<span class="codexdian-interrupted">Interrupted</span> <span class="codexdian-interrupted-hint">· What should Codexdian do instead?</span>');
        }
        streamController.hideThinkingIndicator();
        state.isStreaming = false;
        state.cancelRequested = false;
