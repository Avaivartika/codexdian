import type { CodexService } from '../../../core/agent';
...
export interface StreamControllerDeps {
  ...
  getAgentService?: () => CodexService | null;
}
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.codexdian-tool-name') as HTMLElement | null
            ?? toolEl.querySelector('.codexdian-write-edit-name') as HTMLElement | null;
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.codexdian-tool-summary') as HTMLElement | null
            ?? toolEl.querySelector('.codexdian-write-edit-summary') as HTMLElement | null;
          if (summaryEl) {
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.thinkingIndicatorTimeout = setTimeout(() => {
      ...
      const cls = overrideCls
        ? `codexdian-thinking ${overrideCls}`
        : 'codexdian-thinking';
