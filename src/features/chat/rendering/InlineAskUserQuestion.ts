...
export class InlineAskUserQuestion {
  ...
  private renderQuestionTab(idx: number): void {
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'codexdian-ask-cursor' });
    ...
    if (this.config.showCustomInput) {
      ...
      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'codexdian-ask-cursor' });
  ...
  private renderSubmitTab(): void {
    ...
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'codexdian-ask-cursor' });
    ...
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'codexdian-ask-cursor' });
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.codexdian-ask-check') as HTMLElement | null;
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.codexdian-ask-label-row') as HTMLElement | null;
    ...
    for (let i = 0; i < this.currentItems.length; i++) {
      ...
      if (i === this.focusedItemIndex) {
        ...
        if (item.hasClass('codexdian-ask-custom-item')) {
          const input = item.querySelector('.codexdian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.focus();
            this.isInputFocused = true;
          }
