...
export class MessageRenderer {
  ...
  private renderInterruptMessage(): void {
    ...
    const contentEl = msgEl.createDiv({ cls: 'codexdian-message-content', attr: { dir: 'auto' } });
    const textEl = contentEl.createDiv({ cls: 'codexdian-text-block' });
    textEl.innerHTML = '<span class="codexdian-interrupted">Interrupted</span> <span class="codexdian-interrupted-hint">· What should Codexdian do instead?</span>';
  }
  ...
  async renderContent(el: HTMLElement, markdown: string): Promise<void> {
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('codexdian-code-wrapper')) return;
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector('.codexdian-user-msg-actions') as HTMLElement | null;
