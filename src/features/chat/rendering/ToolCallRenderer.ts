...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.codexdian-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.codexdian-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.codexdian-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.codexdian-tool-current') as HTMLElement;
    if (currentTaskEl) {
