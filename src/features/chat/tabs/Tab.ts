...
import { CodexService } from '../../../core/agent';
import type { McpServerManager } from '../../../core/mcp';
import type { ChatMessage, CodexModel, Conversation, EffortLevel, PermissionMode, SlashCommand, ThinkingBudget } from '../../../core/types';
...
function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  ...
  const viewHeight = textarea.closest('.codexdian-container')?.clientHeight ?? window.innerHeight;
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  ...
  // Nav row (for tab badges and header icons, populated by CodexdianView)
...
/**
 * Initializes the tab's CodexService (lazy initialization).
 ...
 */
function initializeContextManagers(tab: TabData, plugin: CodexdianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: CodexdianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInputToolbar(tab: TabData, plugin: CodexdianPlugin): void {
  const { dom } = tab;
 ...
 */
function resolveForkSource(tab: TabData, plugin: CodexdianPlugin): ForkSource | null {
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: CodexdianPlugin): void {
 ...
 */
export function getTabTitle(tab: TabData, plugin: CodexdianPlugin): string {
  if (tab.conversationId) {
    const conversation = plugin.getConversationSync(tab.conversationId);
...
export function setupServiceCallbacks(tab: TabData, plugin: CodexdianPlugin): void {
...
export function updatePlanModeUI(tab: TabData, plugin: CodexdianPlugin, mode: PermissionMode): void {
  plugin.settings.permissionMode = mode;
