...
import type { HookCallbackMatcher, Options } from '../../core/sdk/compat';
import { query as agentQuery } from '../../core/sdk/compat';
...
export class InlineEditService {
  ...
  private async sendMessage(prompt: string): Promise<InlineEditResult> {
    ...
    const options: Options = {
      ...
      settingSources: this.plugin.settings.loadUserCodexSettings
        ? ['user', 'project']
        : ['project'],
      hooks: {
    ...
    try {
      const response = agentQuery({ prompt, options }) as AsyncIterable<any> & { interrupt?: () => Promise<void> | void };
      let responseText = '';
