...
import type { CodexdianMcpServer, McpServerConfig, McpServerType } from '../../../core/types';
...
export class McpSettingsManager {
  ...
  private servers: CodexdianMcpServer[] = [];
  ...
  private render() {
    ...
    setIcon(stdioOption.createSpan({ cls: 'codexdian-mcp-add-option-icon' }), 'terminal');
    ...
    setIcon(httpOption.createSpan({ cls: 'codexdian-mcp-add-option-icon' }), 'globe');
    ...
    setIcon(importOption.createSpan({ cls: 'codexdian-mcp-add-option-icon' }), 'clipboard-paste');
  ...
  private async testServer(server: CodexdianMcpServer) {
  ...
  private async updateAllDisabledTools(server: CodexdianMcpServer, disabledTools: string[]) {
    await this.updateServerDisabledTools(
      server,
  ...
  private getServerPreview(server: CodexdianMcpServer, type: McpServerType): string {
    if (type === 'stdio') {
  ...
  private openModal(existing: CodexdianMcpServer | null, initialType?: McpServerType) {
  ...
  private async saveServer(server: CodexdianMcpServer, existing: CodexdianMcpServer | null) {
    if (existing) {
  ...
  private async toggleServer(server: CodexdianMcpServer) {
  ...
  private async deleteServer(server: CodexdianMcpServer) {
