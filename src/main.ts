/**
 * Codexdian - Obsidian plugin entry point
 *
...
import { CodexdianView } from './features/chat/CodexdianView';
import { CODEX_ICON_ID } from './features/chat/constants';
import { type InlineEditContext, InlineEditModal } from './features/inline-edit/ui/InlineEditModal';
import { CodexdianSettingTab } from './features/settings/CodexdianSettings';
import { setLocale } from './i18n';
import { CodexCliResolver } from './utils/codexCli';
import { buildCursorContext } from './utils/editor';
import {
  CUSTOM_MODEL_ENV_KEYS,
...
/**
 * Main plugin class for Codexdian.
...
export default class CodexdianPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        ...
        const view = leaf.view as CodexdianView;
  ...
  async loadSettings() {
    ...
    if (!this.settings.codexCliPathsByHost[hostname]) {
      ...
      const migratedPath = platformPaths?.[getCliPlatformKey()]?.trim() || this.settings.codexCliPath?.trim();
  ...
  normalizeModelVariantSettings(): boolean {
    ...
    const normalize = (model: string): string =>
      normalizeVisibleModelVariant(model, enableGPT54HighContext, enableGPT53CodexHighContext);
    ...
    if (this.settings.lastCodexModel) {
      const normalizedLastCodexModel = normalize(this.settings.lastCodexModel);
  ...
  getResolvedCodexCliPath(): string | null {
    return this.cliResolver.resolve(
      this.settings.codexCliPathsByHost,  // Per-device paths (preferred)
      this.settings.codexCliPath,          // Legacy path (fallback)
  ...
  getView(): CodexdianView | null {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_CODEXDIAN);
    if (leaves.length > 0) {
      return leaves[0].view as CodexdianView;
    }
    return null;
  }

  /** Returns all open Codexdian views in the workspace. */
  getAllViews(): CodexdianView[] {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_CODEXDIAN);
    return leaves.map(leaf => leaf.view as CodexdianView);
  }
   ...
   */
  findConversationAcrossViews(conversationId: string): { view: CodexdianView; tabId: string } | null {
    for (const view of this.getAllViews()) {
      const tabManager = view.getTabManager();
      if (!tabManager) continue;
