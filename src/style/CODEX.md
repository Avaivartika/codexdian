...
├── settings/       # base (shared .codexdian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: All classes use `.codexdian-` prefix
- **BEM-lite**: `.codexdian-{block}`, `.codexdian-{block}-{element}`, `.codexdian-{block}--{modifier}`
