/**
 * Codexdian - Environment Utilities
 *
...
const LEGACY_PROVIDER_PREFIX = String.fromCharCode(65, 78, 84, 72, 82, 79, 80, 73, 67);
const LEGACY_MODEL_ENV_KEY = `${LEGACY_PROVIDER_PREFIX}_MODEL`;
const LEGACY_DEFAULT_TIER_HIGH_MODEL_ENV_KEY = `${LEGACY_PROVIDER_PREFIX}_DEFAULT_${String.fromCharCode(79, 80, 85, 83)}_MODEL`;
const LEGACY_DEFAULT_TIER_MEDIUM_MODEL_ENV_KEY = `${LEGACY_PROVIDER_PREFIX}_DEFAULT_${String.fromCharCode(83, 79, 78, 78, 69, 84)}_MODEL`;
const LEGACY_DEFAULT_TIER_FAST_MODEL_ENV_KEY = `${LEGACY_PROVIDER_PREFIX}_DEFAULT_${String.fromCharCode(72, 65, 73, 75, 85)}_MODEL`;
const LEGACY_BASE_URL_ENV_KEY = `${LEGACY_PROVIDER_PREFIX}_BASE_URL`;

export const CUSTOM_MODEL_ENV_KEYS = [
  'CODEX_MODEL',
  'OPENAI_MODEL',
...
const MODEL_TYPE_BY_ENV_KEY: Record<string, string> = {
  CODEX_MODEL: 'model',
  OPENAI_MODEL: 'model',
...
export function getModelsFromEnvironment(envVars: Record<string, string>): { value: string; label: string; description: string }[] {
  ...
  for (const [modelValue, info] of sortedEntries) {
    ...
    const displayTypes = sortedTypes.map(type => typeLabel[type as keyof typeof typeLabel] || type);
