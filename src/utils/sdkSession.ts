/**
 * SDK Session Parser - Parses Codex SDK compatibility layer native session files.
 *
 * The SDK stores sessions in ~/.codex/projects/{vault-path-encoded}/{sessionId}.jsonl
 * Each line is a JSON object with message data.
 *
 * This utility converts SDK native messages to Codexdian's ChatMessage format
...
/** Stored in session JSONL files. Based on Codex SDK compatibility layer internal format. */
 ...
 * Common formats: SDK UUIDs, Codexdian IDs (conv-TIMESTAMP-RANDOM).
 */
