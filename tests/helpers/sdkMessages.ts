type SDKMessage = any;
type SDKSystemMessage = any;
type SDKStatusMessage = any;
type SDKCompactBoundaryMessage = any;
type SDKAssistantMessage = any;
type SDKUserMessage = any;
type SDKPartialAssistantMessage = any;
type SDKToolProgressMessage = any;
type SDKAuthStatusMessage = any;
...
export function buildSystemInitMessage(overrides: Partial<Omit<SDKSystemMessage, 'type' | 'subtype'>> = {}): SDKSystemMessage {
  return {
    ...
    subtype: 'init',
    apiKeySource: 'user',
    codex_code_version: 'test-version',
    cwd: '/test/cwd',
    tools: [],
    mcp_servers: [],
    model: 'gpt-5.3-codex',
