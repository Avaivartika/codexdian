...
describe('McpStorage', () => {
  ...
  describe('load/save', () => {
    it('should preserve unknown top-level keys and merge _codexdian', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const servers: CodexdianMcpServer[] = [
        {
          name: 'new-server',
          config: {
            type: 'http',
      ...
      expect(saved._codexdian).toEqual({
        extra: { keep: true },
        servers: {
          'new-server': {
            enabled: false,
    ...
    it('should keep existing _codexdian fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._codexdian).toEqual({ extra: { keep: true } });
    ...
    it('should remove _codexdian when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
...
describe('McpTester', () => {
  ...
  it('should test stdio server and return tools', async () => {
    const server: CodexdianMcpServer = {
      name: 'local',
...
describe('McpServerManager', () => {
  function createManager(servers: CodexdianMcpServer[]): McpServerManager {
  ...
  describe('getActiveServers', () => {
    ...
    it('should return empty object for all disabled servers', () => {
      const disabledServers: CodexdianMcpServer[] = [
