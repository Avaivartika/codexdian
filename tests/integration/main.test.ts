...
import { DEFAULT_SETTINGS, VIEW_TYPE_CODEXDIAN } from '@/core/types';
import * as sdkSession from '@/utils/sdkSession';

// Mock fs for CodexService
jest.mock('fs');

// Now import the plugin after mocking
import CodexdianPlugin from '@/main';

describe('CodexdianPlugin', () => {
  ...
  beforeEach(() => {
    ...
    plugin = new CodexdianPlugin(mockApp, mockManifest);
  ...
  describe('onload', () => {
    ...
    it('should migrate legacy cli path to hostname-based paths and clear old field', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        // codexCliPath is now in codexdian-settings.json
        return path === '.codex/codexdian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.codex/codexdian-settings.json') {
      ...
      const settingsWrite = (mockApp.vault.adapter.write as jest.Mock).mock.calls.find(
        ([path]) => path === '.codex/codexdian-settings.json'
      );
  ...
  describe('onunload', () => {
    // Note: With multi-tab, cleanup is handled per-tab via CodexdianView.onClose()
  ...
  describe('loadSettings', () => {
    it('should merge saved data with defaults', async () => {
      // Mock codexdian-settings.json exists with custom values (Codexdian-specific settings)
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.codex/codexdian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should normalize blockedCommands when stored value is partial', async () => {
      // Mock codexdian-settings.json exists with partial blockedCommands
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.codex/codexdian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should reconcile model from environment and persist when changed', async () => {
      // Mock codexdian-settings.json with environment variables
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.codex/codexdian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
  ...
  describe('saveSettings', () => {
    it('should save settings to file', async () => {
      ...
      // Codexdian-specific settings should be written to .codex/codexdian-settings.json
      expect(mockApp.vault.adapter.write).toHaveBeenCalledWith(
        '.codex/codexdian-settings.json',
      ...
      // Permissions are now in .codex/settings.json (CC format), not codexdian-settings.json
  ...
  describe('applyEnvironmentVariables', () => {
    ...
    it('invalidates sessions when env hash changes', async () => {
      ...
      await plugin.applyEnvironmentVariables('CODEX_MODEL=gpt-5.3-codex');
  ...
  describe('loadSettings with conversations', () => {
    it('should load saved conversations from JSONL files', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        // Session files
        if (path === '.codex/sessions' || path === '.codex/sessions/conv-saved-1.jsonl') {
          return true;
        }
        // codexdian-settings.json exists
        if (path === '.codex/codexdian-settings.json') {
          return true;
        }
        return false;
      ...
      mockApp.vault.adapter.list.mockImplementation(async (path: string) => {
        if (path === '.codex/sessions') {
          return { files: ['.codex/sessions/conv-saved-1.jsonl'], folders: [] };
        }
        return { files: [], folders: [] };
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.codex/sessions/conv-saved-1.jsonl') {
          return sessionJsonl;
        }
        if (path === '.codex/codexdian-settings.json') {
          return JSON.stringify({});
    ...
    it('should clear session IDs when provider base URL changes', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.codex/codexdian-settings.json' ||
          path === '.codex/sessions' ||
          path === '.codex/sessions/conv-saved-1.jsonl';
      });
      mockApp.vault.adapter.list.mockImplementation(async (path: string) => {
        if (path === '.codex/sessions') {
          return { files: ['.codex/sessions/conv-saved-1.jsonl'], folders: [] };
      ...
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.codex/codexdian-settings.json') {
          // All these fields are now in codexdian-settings.json
          return JSON.stringify({
            lastEnvHash: 'old-hash',
            environmentVariables: 'CODEX_BASE_URL=https://api.example.com',
          });
        }
        if (path === '.codex/sessions/conv-saved-1.jsonl') {
          return sessionJsonl;
      ...
      const sessionWrite = (mockApp.vault.adapter.write as jest.Mock).mock.calls.find(
        ([path]) => path === '.codex/sessions/conv-saved-1.jsonl'
      );
  ...
  describe('Multi-session message loading', () => {
    it('should load messages from previousSdkSessionIds when present', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.codex/codexdian-settings.json' ||
          path === '.codex/sessions' ||
          path === '.codex/sessions/conv-multi-session.meta.json';
      });
      mockApp.vault.adapter.list.mockImplementation(async (path: string) => {
        if (path === '.codex/sessions') {
          return { files: ['.codex/sessions/conv-multi-session.meta.json'], folders: [] };
        }
        return { files: [], folders: [] };
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.codex/sessions/conv-multi-session.meta.json') {
          return sessionMeta;
        }
        if (path === '.codex/codexdian-settings.json') {
          return JSON.stringify({});
