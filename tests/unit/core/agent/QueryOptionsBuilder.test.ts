...
import type { CodexdianSettings } from '@/core/types';
...
function createMockSettings(overrides: Partial<CodexdianSettings> = {}): CodexdianSettings {
  return {
...
describe('QueryOptionsBuilder', () => {
  describe('needsRestart', () => {
    ...
    it('returns false when only model changes (dynamic update)', () => {
      const currentConfig = createMockPersistentQueryConfig();
      const newConfig = { ...currentConfig, model: 'gpt-5.4' };
  ...
  describe('buildPersistentQueryConfig', () => {
    ...
    it('sets settingSources to user,project when loadUserCodexSettings is true', () => {
      const ctx = createMockContext({
        settings: createMockSettings({ loadUserCodexSettings: true }),
      });
      const config = QueryOptionsBuilder.buildPersistentQueryConfig(ctx);
  ...
  describe('buildColdStartQueryOptions', () => {
    ...
    it('uses model override when provided', () => {
      const ctx = {
        ...createMockContext({
          settings: createMockSettings({ model: 'gpt-5.3-codex' }),
        }),
        abortController: new AbortController(),
        hooks: {},
