...
import type { SpawnOptions } from '@/core/sdk/compat';
...
describe('createCustomSpawnFunction', () => {
  ...
  it('pipes stderr only when DEBUG_CODEX_AGENT_SDK is set', () => {
    const mockProcess = createMockProcess();
    spawnMock.mockReturnValue(mockProcess as unknown as ReturnType<typeof spawn>);
  ...
  it('ignores stderr when DEBUG_CODEX_AGENT_SDK is not set', () => {
    const mockProcess = createMockProcess();
    spawnMock.mockReturnValue(mockProcess as unknown as ReturnType<typeof spawn>);
