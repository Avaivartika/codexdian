...
const globalSettingsPath = path.join(homeDir, '.codex', 'settings.json');
const projectSettingsPath = path.join(vaultPath, '.codex', 'settings.json');
  ...
  describe('loadPlugins', () => {
    ...
    it('extracts plugin name from ID correctly', async () => {
      const installedPlugins = {
        version: 2,
        plugins: {
          'feature-dev@codex-plugins-official': [{
            scope: 'user',
