...
describe('transformSDKMessage', () => {
  ...
  describe('result messages', () => {
    ...
    it('matches built-in aliases against SDK modelUsage keys when unambiguous', () => {
      ...
      const results = [...transformSDKMessage(message, { intendedModel: 'gpt-5.4[1m]' })];
    ...
    it('uses intended model context when multi-model result usage includes intended model', () => {
      const message = msg({
        type: 'result',
        modelUsage: {
          'gpt-5.3-codex': {
      ...
      const results = [...transformSDKMessage(message, { intendedModel: 'gpt-5.3-codex' })];

      expect(results).toEqual([{ type: 'context_window_update', contextWindow: 200000 }]);
    });
  });
  ...
  describe('assistant message usage extraction', () => {
    ...
    it('uses custom context limits over standard window', () => {
      ...
      const results = [...transformSDKMessage(message, {
        intendedModel: 'gpt-5.3-codex',
        customContextLimits: { 'gpt-5.3-codex': 256000 },
      })];
