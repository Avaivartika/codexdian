...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _codexdian metadata', async () => {
      const config = {
        mcpServers: {
      ...
      const adapter = createMockAdapter({
        '.codex/mcp.json': JSON.stringify(config),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
    ...
    it('returns empty array when mcpServers is not an object', async () => {
      const adapter = createMockAdapter({
        '.codex/mcp.json': JSON.stringify({ mcpServers: 'invalid' }),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
      expect(servers).toEqual([]);
    ...
    it('applies defaults when no _codexdian metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _codexdian metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _codexdian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      // No _codexdian since all fields are default
      expect(saved._codexdian).toBeUndefined();
    });

    it('preserves existing _codexdian metadata when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const adapter = createMockAdapter({
        '.codex/mcp.json': JSON.stringify(existing),
      });
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian.customField).toBe('should be preserved');
      expect(saved._codexdian.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _codexdian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _codexdian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _codexdian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _codexdian.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _codexdian: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.codex/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _codexdian fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _codexdian: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.codex/mcp.json']);
      expect(saved._codexdian).toEqual({ customField: 'keep' });
    });

    it('handles corrupted existing file gracefully', async () => {
      const adapter = createMockAdapter({
        '.codex/mcp.json': 'not json',
      });
      const storage = new McpStorage(adapter);
