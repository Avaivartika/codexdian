...
describe('SkillStorage', () => {
  it('exports SKILLS_PATH', () => {
    expect(SKILLS_PATH).toBe('.codex/skills');
  });
...
Do the thing`,
    ...
    it('loads multiple skills', async () => {
      const adapter = createMockAdapter({
        '.codex/skills/skill-a/SKILL.md': `---
description: Skill A
---
Prompt A`,
        '.codex/skills/skill-b/SKILL.md': `---
description: Skill B
...
Prompt B`,
    ...
    it('skips folders without SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.codex/skills/has-skill/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codex/skills/no-skill/README.md': 'Just a readme',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
    ...
    it('skips malformed skill and continues loading valid ones', async () => {
      const adapter = createMockAdapter({
        '.codex/skills/good/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codex/skills/bad/SKILL.md': 'content',
      });
...
Full prompt`,
      ...
      expect(skill.model).toBe('gpt-5.3-codex');
    ...
    it('loads skills without frontmatter as content-only', async () => {
      const adapter = createMockAdapter({
        '.codex/skills/valid/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codex/skills/invalid/SKILL.md': 'No frontmatter at all',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
  ...
  describe('save', () => {
    it('writes skill to correct path', async () => {
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.codex/skills/my-skill');
