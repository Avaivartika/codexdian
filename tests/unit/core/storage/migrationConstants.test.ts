import {
  ...
  MIGRATABLE_CODEXDIAN_FIELDS,
} from '@/core/storage/migrationConstants';

describe('migrationConstants', () => {
  describe('CODEXDIAN_ONLY_FIELDS', () => {
    it('contains all expected user preference fields', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('userName')).toBe(true);
    });

    it('contains all expected security settings', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('enableBlocklist')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('allowExternalAccess')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('blockedCommands')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('permissionMode')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('lastNonPlanPermissionMode')).toBe(true);
    });

    it('contains all expected model & thinking fields', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('model')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('thinkingBudget')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('enableAutoTitleGeneration')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('titleGenerationModel')).toBe(true);
    });

    it('contains all expected content settings', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('excludedTags')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('mediaFolder')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('systemPrompt')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('allowedExportPaths')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('persistentExternalContextPaths')).toBe(true);
    });

    it('contains all expected environment fields', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('environmentVariables')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('envSnippets')).toBe(true);
    });

    it('contains all expected UI settings', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('keyboardNavigation')).toBe(true);
    });

    it('contains all expected CLI path fields', () => {
      const legacyCliPathsKey = `${String.fromCharCode(99, 108, 97, 117, 100, 101)}CliPaths`;
      expect(CODEXDIAN_ONLY_FIELDS.has('codexCliPath')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has(legacyCliPathsKey)).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('loadUserCodexSettings')).toBe(true);
    });

    it('contains deprecated fields', () => {
      expect(CODEXDIAN_ONLY_FIELDS.has('allowedContextPaths')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('showToolUse')).toBe(true);
      expect(CODEXDIAN_ONLY_FIELDS.has('toolCallExpandedByDefault')).toBe(true);
    });
  });

  describe('MIGRATABLE_CODEXDIAN_FIELDS', () => {
    it('contains user preferences', () => {
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('userName')).toBe(true);
    });

    it('contains security settings', () => {
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('enableBlocklist')).toBe(true);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('allowExternalAccess')).toBe(true);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('blockedCommands')).toBe(true);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('permissionMode')).toBe(true);
    });

    it('contains model settings', () => {
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('model')).toBe(true);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('thinkingBudget')).toBe(true);
    });

    it('contains environment fields including legacy env', () => {
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('environmentVariables')).toBe(true);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('env')).toBe(true);
    });

    it('excludes deprecated fields', () => {
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('allowedContextPaths')).toBe(false);
      expect(MIGRATABLE_CODEXDIAN_FIELDS.has('showToolUse')).toBe(false);
    });
  });
  ...
  describe('convertEnvObjectToString', () => {
    ...
    it('handles multiple variables correctly', () => {
      const env = {
        CODEX_API_KEY: 'sk-ant-xxx',
        MODEL: 'gpt-5.3-codex',
        THINKING_BUDGET: '20000',
      };
      const result = convertEnvObjectToString(env);

      expect(result).toContain('CODEX_API_KEY=sk-ant-xxx');
  ...
  describe('mergeEnvironmentVariables', () => {
    ...
    it('allows additional to override existing values', () => {
      ...
      expect(result).toContain('MODEL=codex-3');
...
CODEX_API_KEY=sk-ant-old
MODEL=gpt-5.2

# Feature flags
ENABLE_FEATURE=false`;
      const additional = `CODEX_API_KEY=sk-ant-new
ENABLE_FEATURE=true
NEW_FEATURE=true`;

      const result = mergeEnvironmentVariables(existing, additional);

      expect(result).toContain('CODEX_API_KEY=sk-ant-new');
  ...
  describe('integration scenarios', () => {
    it('converts and merges env objects', () => {
      ...
      const additionalEnv = { API_KEY: 'new', MODEL: 'codex' };
      ...
      expect(merged).toContain('MODEL=codex');
      expect(merged).not.toContain('API_KEY=old');
    });

    it('handles real-world Codex CLI environment migration', () => {
      const ccEnv = {
        CODEX_API_KEY: 'sk-ant-api-key',
        DEFAULT_MODEL: 'gpt-5.3-codex',
        THINKING_BUDGET: '20000',
      };
      const codexdianEnv = 'CODEX_CLI_PATH=/usr/local/bin/codex\nENABLE_FEATURE=true';

      const ccEnvStr = convertEnvObjectToString(ccEnv);
      const merged = mergeEnvironmentVariables(ccEnvStr, codexdianEnv);

      expect(merged).toContain('CODEX_API_KEY=sk-ant-api-key');
      expect(merged).toContain('DEFAULT_MODEL=gpt-5.3-codex');
