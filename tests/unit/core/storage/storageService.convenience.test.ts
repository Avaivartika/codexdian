...
describe('StorageService convenience methods', () => {
  ...
  const codexdianSettingsJson = JSON.stringify({
    userName: 'Test',
  ...
  describe('getPermissions', () => {
    it('delegates to ccSettings.getPermissions', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codex/settings.json': ccSettingsJson,
        },
      });
  ...
  describe('updatePermissions', () => {
    it('saves updated permissions via ccSettings', async () => {
      ...
      const saved = JSON.parse(files.get('.codex/settings.json')!) as Record<string, unknown>;
  ...
  describe('removePermissionRule', () => {
    it('removes a rule from all permission lists', async () => {
      ...
      const { plugin, files } = createMockPlugin({
        initialFiles: { '.codex/settings.json': settings },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
  ...
  describe('updateCodexdianSettings', () => {
    it('updates partial codexdian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.codex/codexdian-settings.json': codexdianSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();

      await storage.updateCodexdianSettings({ userName: 'NewUser' });
  ...
  describe('saveCodexdianSettings', () => {
    it('saves full codexdian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.codex/codexdian-settings.json': codexdianSettingsJson,
        },
      ...
      const existing = await storage.loadCodexdianSettings();
      existing.userName = 'FullSave';
      await storage.saveCodexdianSettings(existing);
  ...
  describe('loadCodexdianSettings', () => {
    it('loads codexdian settings', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codex/codexdian-settings.json': codexdianSettingsJson,
        },
      ...
      const settings = await storage.loadCodexdianSettings();
  ...
  describe('loadAllSlashCommands', () => {
    it('returns commands from both commands and skills directories', async () => {
      ...
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codex/commands/review.md': commandContent,
          '.codex/skills/my-skill/SKILL.md': skillContent,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
  ...
  describe('getLegacyActiveConversationId', () => {
    it('returns id from codexdian settings', async () => {
      const settings = JSON.stringify({
        userName: 'Test',
      ...
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codex/codexdian-settings.json': settings,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
    ...
    it('falls back to data.json when not in codexdian settings', async () => {
      const { plugin } = createMockPlugin({
        dataJson: { activeConversationId: 'conv-from-data' },
        initialFiles: {
          '.codex/codexdian-settings.json': JSON.stringify({ userName: 'Test' }),
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
