...
describe('StorageService migration', () => {
  it('clears data.json after successful legacy content migration', async () => {
    ...
    expect(files.has('.codex/commands/review.md')).toBe(true);
  ...
  it('does not clear data.json when legacy content migration fails', async () => {
    ...
    const { plugin } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      shouldFailWrite: (path) => path.startsWith('.codex/commands/'),
    });
  ...
  it('normalizes legacy blockedCommands during settings migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: null,
      initialFiles: {
        '.codex/settings.json': JSON.stringify(legacySettings),
      },
    });
  ...
  it('does not migrate legacy activeConversationId from data.json', async () => {
    ...
    const rawSettings = files.get('.codex/codexdian-settings.json');
  ...
  it('preserves CC-format permissions during migration', async () => {
    ...
    const ccSettings = JSON.parse(files.get('.codex/settings.json') || '{}') as Record<string, any>;
  ...
  it('migrates data.json state fields to codexdian-settings when empty', async () => {
    // Migration only writes when the target field is falsy.
    // Default lastCodexModel='gpt-5.4' (truthy) → won't overwrite
    // Default lastCustomModel='' (falsy) → will overwrite
    // Default lastEnvHash='' (falsy) → will overwrite
    const { plugin, files } = createMockPlugin({
      dataJson: {
        lastEnvHash: 'abc123',
        lastCodexModel: 'codex-3-gpt-5.3-codex',
  ...
  it('does not overwrite existing codexdian-settings fields from data.json', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
      ...
      initialFiles: {
        '.codex/codexdian-settings.json': JSON.stringify({
          userName: 'Test User',
  ...
  it('skips existing slash commands during migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      initialFiles: {
        '.codex/commands/review.md': 'Existing content',
      },
    });
    ...
    expect(files.get('.codex/commands/review.md')).toBe('Existing content');
  });
  ...
  it('migrates conversations from data.json', async () => {
    ...
    expect(files.has('.codex/sessions/conv-1.jsonl')).toBe(true);
  });
  ...
  it('skips existing conversations during migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: { conversations: [conversation] },
      initialFiles: {
        '.codex/sessions/conv-1.jsonl': '{"existing": true}',
      },
    });
    ...
    expect(files.get('.codex/sessions/conv-1.jsonl')).toBe('{"existing": true}');
  });
  ...
  it('migrates lastCodexModel from data.json when codexdian-settings has falsy value', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
        lastCodexModel: 'codex-3-gpt-5.3-codex',
      },
    ...
    const saved = JSON.parse(files.get('.codex/codexdian-settings.json') || '{}') as Record<string, unknown>;
    expect(saved.lastCodexModel).toBe('codex-3-gpt-5.3-codex');
  });
  ...
  it('preserves persistentExternalContextPaths from existing settings', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: null,
      initialFiles: {
        '.codex/codexdian-settings.json': JSON.stringify(existingSettings),
      },
    });
