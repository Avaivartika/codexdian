...
describe('InputController - Message Queue', () => {
  ...
  describe('handleApprovalRequest', () => {
    ...
    it.each([
      ...
      const items = parentEl.querySelectorAll('codexdian-ask-item');
      const target = items.find((item: any) => {
        const label = item.querySelector('codexdian-ask-item-label');
    ...
    it('should render header metadata when approvalOptions provided', async () => {
      ...
      const reasonEl = parentEl.querySelector('codexdian-ask-approval-reason');
      expect(reasonEl?.textContent).toBe('Command is destructive');

      const pathEl = parentEl.querySelector('codexdian-ask-approval-blocked-path');
      expect(pathEl?.textContent).toBe('/usr/bin/rm');

      const agentEl = parentEl.querySelector('codexdian-ask-approval-agent');
