...
function findRoot(container: any): any {
  return container.querySelector('.codexdian-plan-approval-inline');
}

function findItems(root: any): any[] {
  return root.querySelectorAll('codexdian-ask-item');
}

describe('InlineExitPlanMode', () => {
  it('resolves with approve-new-session and includes plan content when readable', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexdian-'));
    ...
    expect(container.querySelector('.codexdian-plan-permissions-list')).toBeTruthy();
  ...
  it('shows a read error when plan file cannot be read', () => {
    ...
    const root = findRoot(container);
    expect(root).toBeTruthy();
    expect(container.querySelector('.codexdian-plan-read-error')).toBeTruthy();
  ...
  it('supports feedback flow and Escape when input is focused', () => {
    ...
    const feedbackInput = feedbackRow.querySelector('codexdian-ask-custom-text');
