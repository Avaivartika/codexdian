...
jest.mock('@/core/agent', () => ({
  CodexService: jest.fn().mockImplementation(() => ({
...
const createMockCodexdianService = (overrides?: {
...
describe('Tab - Service Initialization', () => {
  describe('initializeTabService', () => {
    ...
    it('should create CodexService on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should ensureReady without session ID (just spin up process)', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { CodexService: jest.Mock };
      agentModule.CodexService.mockImplementationOnce(() => createMockCodexdianService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync model selector ready state with service readiness', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { CodexService: jest.Mock };
      agentModule.CodexService.mockImplementationOnce(() => createMockCodexdianService({ onReadyStateChange: mockOnReadyStateChange }));
