...
describe('ModelSelector', () => {
  ...
  it('should create a container with model-selector class', () => {
    const container = parentEl.querySelector('.codexdian-model-selector');
  ...
  it('should render model options in reverse order', () => {
    const dropdown = parentEl.querySelector('.codexdian-model-dropdown');
  ...
  it('should call onModelChange when option clicked', async () => {
    const dropdown = parentEl.querySelector('.codexdian-model-dropdown');
    const options = dropdown?.children || [];
    const option = options.find((o: any) => o.children[0]?.textContent === 'GPT-5.2');

    await option?.dispatchEvent('click', { stopPropagation: () => {} });
    expect(callbacks.onModelChange).toHaveBeenCalledWith('gpt-5.2');
  });
  ...
  it('should use custom models from environment variables', () => {
    callbacks.getEnvironmentVariables.mockReturnValue(
      'CODEX_CODE_USE_BEDROCK=1\nCODEX_MODEL=us.openai.gpt-5.3-codex'
    );
    callbacks.getSettings.mockReturnValue({
      model: 'us.openai.gpt-5.3-codex',
      thinkingBudget: 'low',
      permissionMode: 'normal',
      enableGPT54HighContext: false,
      enableGPT53CodexHighContext: false,
  ...
  it('should not filter custom env models when 1M toggles are enabled', () => {
    ...
    callbacks.getSettings.mockReturnValue({
      ...
      thinkingBudget: 'low',
      permissionMode: 'normal',
      enableGPT54HighContext: true,
      enableGPT53CodexHighContext: true,
    });
  ...
  it('should show 1M variants instead of standard variants when enabled', () => {
    ...
    expect(parentEl.querySelector('.codexdian-model-label')?.textContent).toBe('GPT-5.3 Codex');
  });
});
...
describe('ThinkingBudgetSelector', () => {
  ...
  describe('adaptive mode (Codex models)', () => {
    beforeEach(() => {
      jest.clearAllMocks();
      parentEl = createMockEl();
      callbacks = createMockCallbacks();
    ...
    it('should create a container with thinking-selector class', () => {
      const container = parentEl.querySelector('.codexdian-thinking-selector');
      expect(container).not.toBeNull();
    });

    it('should show effort selector for Codex models', () => {
      const effort = parentEl.querySelector('.codexdian-thinking-effort');
      expect(effort).not.toBeNull();
      expect(effort?.style?.display).not.toBe('none');
    });

    it('should hide budget selector for Codex models', () => {
      const budget = parentEl.querySelector('.codexdian-thinking-budget');
      expect(budget?.style?.display).toBe('none');
    });

    it('should display current effort level for Codex models', () => {
      const current = parentEl.querySelector('.codexdian-thinking-current');
  ...
  describe('legacy mode (custom models)', () => {
    ...
    it('should render budget options in reverse order', () => {
      const options = parentEl.querySelector('.codexdian-thinking-options');
...
describe('PermissionToggle', () => {
  ...
  it('should create a container with permission-toggle class', () => {
    const container = parentEl.querySelector('.codexdian-permission-toggle');
    expect(container).not.toBeNull();
  });

  it('should display Safe label when in normal mode', () => {
    const label = parentEl.querySelector('.codexdian-permission-label');
  ...
  it('should display YOLO label when in yolo mode', () => {
    ...
    const label = parentEl2.querySelector('.codexdian-permission-label');
  ...
  it('should show PLAN label and hide toggle in plan mode', () => {
    ...
    const toggle = parentEl2.querySelector('.codexdian-toggle-switch');
  ...
  it('should not have active class in normal mode', () => {
    const toggle = parentEl.querySelector('.codexdian-toggle-switch');
...
describe('McpServerSelector', () => {
  ...
  it('should create container with mcp-selector class', () => {
    const container = parentEl.querySelector('.codexdian-mcp-selector');
    expect(container).not.toBeNull();
  });
  ...
  it('should show empty message when all servers are disabled', () => {
    ...
    const empty = parentEl.querySelector('.codexdian-mcp-selector-empty');
  ...
  it('should show badge when more than 1 server enabled', () => {
    ...
    const badge = parentEl.querySelector('.codexdian-mcp-selector-badge');
  ...
  it('should add active class to icon when servers are enabled', () => {
    ...
    const icon = parentEl.querySelector('.codexdian-mcp-selector-icon');
...
describe('ContextUsageMeter', () => {
  ...
  it('should create a container with context-meter class', () => {
    const container = parentEl.querySelector('.codexdian-context-meter');
    expect(container).not.toBeNull();
  });
  ...
  it('should display percentage', () => {
    meter.update(makeUsage({ contextTokens: 50000, contextWindow: 200000, percentage: 25 }));
    const percent = parentEl.querySelector('.codexdian-context-meter-percent');
...
describe('McpServerSelector - toggle and badges', () => {
  ...
  it('should render context-saving badge for servers with contextSaving', () => {
    ...
    const csBadge = parentEl.querySelector('.codexdian-mcp-selector-cs-badge');
  ...
  it('should toggle server on mousedown and update display', () => {
    ...
    const item = parentEl.querySelector('.codexdian-mcp-selector-item');
