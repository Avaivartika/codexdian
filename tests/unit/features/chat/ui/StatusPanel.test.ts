...
class MockElement {
  ...
  querySelectorAll(selector: string): MockElement[] {
    ...
    const match = (el: MockElement): boolean => {
      ...
      // Handle class selectors like .codexdian-status-panel
...
describe('StatusPanel', () => {
  ...
  describe('mount', () => {
    it('should create panel element when mounted', () => {
      ...
      expect(containerEl.querySelector('.codexdian-status-panel')).not.toBeNull();
    });

    it('should create hidden todo container initially', () => {
      panel.mount(containerEl as unknown as HTMLElement);

      const todoContainer = containerEl.querySelector('.codexdian-status-panel-todos');
  ...
  describe('updateTodos', () => {
    ...
    it('should display correct task count', () => {
      ...
      const label = containerEl.querySelector('.codexdian-status-panel-label');
    ...
    it('should show current task in collapsed header', () => {
      ...
      const current = containerEl.querySelector('.codexdian-status-panel-current');
    ...
    it('should render all todo items in content area', () => {
      ...
      const items = containerEl.querySelectorAll('.codexdian-todo-item');
    ...
    it('should apply correct status classes to items', () => {
      ...
      expect(containerEl.querySelector('.codexdian-todo-pending')).not.toBeNull();
      expect(containerEl.querySelector('.codexdian-todo-in_progress')).not.toBeNull();
      expect(containerEl.querySelector('.codexdian-todo-completed')).not.toBeNull();
    });
  ...
  describe('toggle', () => {
    ...
    it('should expand content on header click', () => {
      const header = containerEl.querySelector('.codexdian-status-panel-header');
      const content = containerEl.querySelector('.codexdian-status-panel-content');
    ...
    it('should show list icon in header', () => {
      const icon = containerEl.querySelector('.codexdian-status-panel-icon');
    ...
    it('should hide current task when expanded', () => {
      ...
      expect(containerEl.querySelector('.codexdian-status-panel-current')).not.toBeNull();

      header!.click();

      expect(containerEl.querySelector('.codexdian-status-panel-current')).toBeNull();
    });
  ...
  describe('accessibility', () => {
    ...
    it('should hide status icons from screen readers', () => {
      ...
      const icon = containerEl.querySelector('.codexdian-todo-status-icon');
  ...
  describe('remount', () => {
    ...
    it('should re-render subagents after remount', () => {
      ...
      const subagentsEl = containerEl.querySelector('.codexdian-status-panel-subagents');
  ...
  describe('removeSubagent', () => {
    ...
    it('should remove a subagent by id', () => {
      ...
      const runningText = containerEl.querySelector('.codexdian-status-panel-running-text');
  ...
  describe('truncateDescription', () => {
    ...
    it('should truncate long subagent descriptions', () => {
      ...
      const doneText = containerEl.querySelector('.codexdian-status-panel-done-text');
  ...
  describe('destroy', () => {
    it('should remove panel from DOM', () => {
      ...
      expect(containerEl.querySelector('.codexdian-status-panel')).toBeNull();
    });
  ...
  describe('updateSubagent', () => {
    ...
    it('should show container when subagent is added', () => {
      ...
      const containerEl2 = containerEl.querySelector('.codexdian-status-panel-subagents');
    ...
    it('should show done row when status changes to completed', () => {
      ...
      const doneRow = containerEl.querySelector('.codexdian-status-panel-done-row');
    ...
    it('should show running row for running status', () => {
      ...
      const runningRow = containerEl.querySelector('.codexdian-status-panel-running-row');
  ...
  describe('subagent status display', () => {
    ...
    it('should show done rows for completed and running row for running', () => {
      ...
      const doneRows = containerEl.querySelectorAll('.codexdian-status-panel-done-row');
    ...
    it('should update display when subagent status changes', () => {
      ...
      let runningText = containerEl.querySelector('.codexdian-status-panel-running-text');
      expect(runningText?.textContent).toBe('1 background task');
      ...
      runningText = containerEl.querySelector('.codexdian-status-panel-running-text');
  ...
  describe('bash outputs', () => {
    ...
    it('should render bash section with header and entries', () => {
      ...
      const bashContainer = containerEl.querySelector('.codexdian-status-panel-bash');
      expect(bashContainer).not.toBeNull();
      expect(bashContainer!.style.display).toBe('block');

      const header = containerEl.querySelector('.codexdian-status-panel-bash-header');
      expect(header).not.toBeNull();
      const label = header!.querySelector('.codexdian-tool-label');
      expect(label).not.toBeNull();
      expect(label!.textContent).toBe('Command panel');

      const entries = containerEl.querySelectorAll('.codexdian-status-panel-bash-entry');
    ...
    it('should collapse and expand the bash section', () => {
      ...
      const content = containerEl.querySelector('.codexdian-status-panel-bash-content');
      ...
      header!.click();
      expect(content!.style.display).toBe('none');
      const collapsedHeader = containerEl.querySelector('.codexdian-status-panel-bash-header');
      expect(collapsedHeader).not.toBeNull();
      const collapsedLabel = collapsedHeader!.querySelector('.codexdian-tool-label');
      expect(collapsedLabel).not.toBeNull();
      expect(collapsedLabel!.textContent).toBe('echo hello');

      header!.click();
      expect(content!.style.display).toBe('block');
      const expandedHeaderAgain = containerEl.querySelector('.codexdian-status-panel-bash-header');
      expect(expandedHeaderAgain).not.toBeNull();
      const expandedLabelAgain = expandedHeaderAgain!.querySelector('.codexdian-tool-label');
    ...
    it('should collapse and expand individual bash output entries', () => {
      ...
      const entry = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      expect(entry).not.toBeNull();

      const entryHeader = entry!.querySelector('.codexdian-tool-header');
      const entryContent = entry!.querySelector('.codexdian-tool-content');
      ...
      const entryAfterClick = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      const contentAfterClick = entryAfterClick!.querySelector('.codexdian-tool-content');
      const headerAfterClick = entryAfterClick!.querySelector('.codexdian-tool-header');
      ...
      const entryAfterKeydown = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      const contentAfterKeydown = entryAfterKeydown!.querySelector('.codexdian-tool-content');
      const headerAfterKeydown = entryAfterKeydown!.querySelector('.codexdian-tool-header');
    ...
    it('should clear bash outputs via action button', () => {
      ...
      const clearButton = containerEl.querySelector('.codexdian-status-panel-bash-action-clear');
    ...
    it('should copy latest bash output via action button', async () => {
      ...
      const copyButton = containerEl.querySelector('.codexdian-status-panel-bash-action-copy');
    ...
    it('should update a running bash output to completed with output', () => {
      ...
      let entry = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      let text = entry!.querySelector('.codexdian-tool-result-text');
      expect(text!.textContent).toBe('Running...');

      panel.updateBashOutput('bash-1', { status: 'completed', output: 'hello', exitCode: 0 });

      entry = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      text = entry!.querySelector('.codexdian-tool-result-text');
      expect(text!.textContent).toBe('hello');

      const statusEl = entry!.querySelector('.codexdian-tool-status');
    ...
    it('should update a running bash output to error', () => {
      ...
      const entry = containerEl.querySelector('.codexdian-status-panel-bash-entry');
      const text = entry!.querySelector('.codexdian-tool-result-text');
    ...
    it('should set aria-expanded on the bash section header', () => {
      ...
      header!.click();
      const headerAfterCollapse = containerEl.querySelector('.codexdian-status-panel-bash-header');
      expect(headerAfterCollapse!.getAttribute('aria-expanded')).toBe('false');

      headerAfterCollapse!.click();
      const headerAfterExpand = containerEl.querySelector('.codexdian-status-panel-bash-header');
