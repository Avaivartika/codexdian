 ...
 * Each rendered item has a `.codexdian-slash-name` span (text: `/{name}`)
 * and an optional `.codexdian-slash-desc` div.
 */
function getRenderedItems(containerEl: any): { name: string; description: string }[] {
  const dropdownEl = containerEl.children.find(
    (c: any) => c.hasClass('codexdian-slash-dropdown')
  );
  if (!dropdownEl) return [];
  const items = dropdownEl.querySelectorAll('.codexdian-slash-item');
  return items.map((item: any) => {
    const nameSpan = item.children.find((c: any) => c.hasClass('codexdian-slash-name'));
    const descDiv = item.children.find((c: any) => c.hasClass('codexdian-slash-desc'));
    return {
...
// Commands that should be filtered out (not shown in Codexdian)
