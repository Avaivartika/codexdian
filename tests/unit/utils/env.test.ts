...
describe('getCustomModelIds', () => {
  ...
  it('should extract model from default tier env vars', () => {
    const result = getCustomModelIds({
      CODEX_DEFAULT_GPT54_MODEL: 'my-gpt-5.4',
      CODEX_DEFAULT_GPT53_CODEX_MODEL: 'my-gpt-5.3-codex',
      CODEX_DEFAULT_GPT51_MINI_MODEL: 'my-gpt-5.1-codex-mini',
    });
    expect(result.size).toBe(3);
    expect(result.has('my-gpt-5.4')).toBe(true);
    expect(result.has('my-gpt-5.3-codex')).toBe(true);
    expect(result.has('my-gpt-5.1-codex-mini')).toBe(true);
  });

  it('should deduplicate when multiple env vars point to same model', () => {
    const result = getCustomModelIds({
      CODEX_MODEL: 'shared-model',
      CODEX_DEFAULT_GPT53_CODEX_MODEL: 'shared-model',
    });
    expect(result.size).toBe(1);
  ...
  it('should ignore empty string model values', () => {
    const result = getCustomModelIds({
      CODEX_MODEL: '',
      CODEX_DEFAULT_GPT53_CODEX_MODEL: 'valid-model',
    });
    expect(result.size).toBe(1);
...
describe('getModelsFromEnvironment', () => {
  ...
  it('returns model for OPENAI_MODEL', () => {
    const result = getModelsFromEnvironment({ OPENAI_MODEL: 'gpt-5.4' });
    expect(result).toHaveLength(1);
    expect(result[0].value).toBe('gpt-5.4');
    expect(result[0].description).toContain('model');
  });

  it('formats label from hyphenated model name', () => {
    const result = getModelsFromEnvironment({ CODEX_MODEL: 'codex-3-gpt-5.4' });
  ...
  it('returns models for tier-specific env vars', () => {
    ...
    expect(result.map(m => m.value)).toContain('my-gpt-5.4');
    expect(result.map(m => m.value)).toContain('my-gpt-5.3-codex');
    expect(result.map(m => m.value)).toContain('my-gpt-5.1-codex-mini');
  });
...
describe('getCurrentModelFromEnvironment', () => {
  ...
  it('prefers OPENAI_MODEL over tier-specific vars', () => {
    expect(getCurrentModelFromEnvironment({
      OPENAI_MODEL: 'gpt-5.4',
      CODEX_DEFAULT_GPT53_CODEX_MODEL: 'gpt-5.3-codex',
      CODEX_DEFAULT_GPT54_MODEL: 'gpt-5.4-legacy',
    })).toBe('gpt-5.4');
  });

  it('prefers CODEX_MODEL over tier-specific vars', () => {
    expect(getCurrentModelFromEnvironment({
      CODEX_MODEL: 'main-model',
