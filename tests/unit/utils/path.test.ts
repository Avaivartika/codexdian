...
describe('getPathAccessType', () => {
  ...
  it('returns vault for ~/.codex safe subdirectory', () => {
    ...
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'projects', 'test'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'commands', 'cmd.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'agents', 'agent.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'skills', 'skill'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'plans', 'plan.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'mcp.json'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'codexdian-settings.json'), [], [], vaultPath)).toBe('vault');
  });
...
describe('findCodexCLIPath', () => {
  ...
  it('resolves from custom path entries', () => {
    ...
    jest.spyOn(fs, 'existsSync').mockImplementation(
      p => String(p) === codexPath
    );
    jest.spyOn(fs, 'statSync').mockImplementation(
      p => ({ isFile: () => String(p) === codexPath }) as fs.Stats
    );
  ...
  it('falls back to PATH environment when common and npm paths fail', () => {
    const envCodexPath = '/env/specific/bin/codex';
    const originalPath = process.env.PATH;
    process.env.PATH = `/env/specific/bin:${originalPath}`;

    jest.spyOn(fs, 'existsSync').mockImplementation(
      p => String(p) === envCodexPath
    );
    jest.spyOn(fs, 'statSync').mockImplementation(
      p => ({ isFile: () => String(p) === envCodexPath }) as fs.Stats
    );

    try {
      const result = findCodexCLIPath();
      expect(result).toBe(envCodexPath);
    } finally {
