...
describe('utils.ts', () => {
  ...
  describe('expandHomePath', () => {
    const envKey = 'CODEXDIAN_TEST_PATH';
    const envValue = path.join(os.tmpdir(), 'codexdian-env');
    ...
    it('should leave unknown environment variables untouched', () => {
      expect(expandHomePath('%CODEXDIAN_MISSING_VAR%')).toBe('%CODEXDIAN_MISSING_VAR%');
      expect(expandHomePath('$CODEXDIAN_MISSING_VAR')).toBe('$CODEXDIAN_MISSING_VAR');
    });
  });
  ...
  describe('normalizePathForFilesystem', () => {
    ...
    it('expands environment variables before filesystem use', () => {
      const envKey = 'CODEXDIAN_FS_TEST_PATH';
      const originalValue = process.env[envKey];
      process.env[envKey] = '/tmp/codexdian-test';

      try {
        expect(normalizePathForFilesystem(`$${envKey}/notes/file.md`)).toBe('/tmp/codexdian-test/notes/file.md');
      } finally {
        if (originalValue === undefined) {
          delete process.env[envKey];
        } else {
    ...
    it('handles chained home and environment variable expansions', () => {
      const envKey = 'CODEXDIAN_TEST_SUBDIR';
      const originalValue = process.env[envKey];
  ...
  describe('getModelsFromEnvironment', () => {
    it('should extract model from CODEX_MODEL', () => {
      const envVars = { CODEX_MODEL: 'codex-3-gpt-5.4' };
      const result = getModelsFromEnvironment(envVars);

      expect(result).toHaveLength(1);
      expect(result[0].value).toBe('codex-3-gpt-5.4');
      expect(result[0].description).toContain('model');
    });

    it('should extract models from OPENAI_DEFAULT_*_MODEL variables', () => {
      const envVars = {
        CODEX_DEFAULT_GPT54_MODEL: 'custom-gpt-5.4',
        CODEX_DEFAULT_GPT53_CODEX_MODEL: 'custom-gpt-5.3-codex',
        CODEX_DEFAULT_GPT51_MINI_MODEL: 'custom-gpt-5.1-codex-mini',
      };
      const result = getModelsFromEnvironment(envVars);

      expect(result).toHaveLength(3);
      expect(result.map(m => m.value)).toContain('custom-gpt-5.4');
      expect(result.map(m => m.value)).toContain('custom-gpt-5.3-codex');
      expect(result.map(m => m.value)).toContain('custom-gpt-5.1-codex-mini');
    });

    it('should deduplicate models with same value', () => {
      const envVars = {
        CODEX_MODEL: 'same-model',
        CODEX_DEFAULT_GPT54_MODEL: 'same-model',
      };
      const result = getModelsFromEnvironment(envVars);
      ...
      expect(result[0].description).toContain('model');
      expect(result[0].description).toContain('gpt-5.4');
    });
    ...
    it('should handle model names with slashes (provider/model format)', () => {
      const envVars = { CODEX_MODEL: 'openai/codex-3-gpt-5.4' };
      const result = getModelsFromEnvironment(envVars);

      expect(result).toHaveLength(1);
      expect(result[0].value).toBe('openai/codex-3-gpt-5.4');
      expect(result[0].label).toBe('codex-3-gpt-5.4');
    });
  ...
  describe('getCurrentModelFromEnvironment', () => {
    ...
    it('should prefer CODEX_MODEL over OPENAI_MODEL when both are set', () => {
      const envVars = {
        CODEX_MODEL: 'main-model',
  ...
  describe('findCodexCLIPath', () => {
    ...
    describe('on Unix/macOS', () => {
      ...
      it('should return first matching Codex CLI path', () => {
  ...
  describe('getPathAccessType', () => {
    ...
    it('should allow access to ~/.codex/ via tilde expansion', () => {
      ...
      expect(getPathAccessType('~/.codex/sessions/abc.jsonl', [], [], '/vault')).toBe('vault');
    });
